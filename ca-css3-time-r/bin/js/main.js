...
var $, Game, HEIGHT, WIDTH, bird, dt, eDebug, game, loadImageAndDraw, man, test, userAgent, util;
...
loadImageAndDraw = function() {
  bird = new Image;
  bird.style.webkitTransform = "translate3d(0, 0, 0)";
  bird.addEventListener("load", function() {
    return game.add(man);
  });
  bird.src = "assets/bird.png";
  return document.body.appendChild(bird);
};
...
man = {
  ...
  update: function() {
    if ((this.x > WIDTH - bird.width) || (this.x < 0)) {
      if (this.x < 0) {
        this.x = 0;
      } else {
        this.x = WIDTH - bird.width;
      }
      this.vx = -this.vx;
    }
    if ((this.y > HEIGHT - bird.height) || (this.y < 0)) {
      if (this.y < 0) {
        this.y = 0;
      } else {
        this.y = HEIGHT - bird.height;
      }
      this.vy = -this.vy;
    }
    this.x += this.vx;
  ...
  draw: function() {
    return bird.style.webkitTransform = "translate3d(" + this.x + "px, " + this.y + "px, 0)";
  }
};

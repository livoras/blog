...
FancySlide = (function(_super) {
  ...
  FancySlide.prototype._initEvents = function() {
    var PER_HEIGHT, previousPos, swiped, swiping;
    PER_HEIGHT = 30;
    previousPos = -1;
    swiping = (function(_this) {
      return function(timeline, page, dist) {
        ...
        if (!_this.able || _this.isAnimating || _this._isTimelineActive()) {
          return;
        }
        move = function() {
          var duration;
          duration = dist / HEIGHT * _this.duration;
          if (!_this.isCss3Set) {
            _this._enableAnimation(duration);
          }
          return timeline.progress(dist / HEIGHT);
        };
        currentPos = Math.floor(dist / PER_HEIGHT);
        clearTimeout(_this.timer);
        if (currentPos > 1 && currentPos !== previousPos) {
          previousPos = currentPos;
          return move();
        }
        return _this.timer = setTimeout(move, 20);
      };
    })(this);
